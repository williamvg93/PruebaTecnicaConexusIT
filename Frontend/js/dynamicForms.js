...
const CheckData = (dataList) => {
  const strData = ["Nombre"];
  const intData = ["Precio", "PrecioUnitario"];
  const idData = [
    "trasportId",
    "StockDisponible",
    "IdCliente",
  ...
  const alphaData = ["NumeroTelefono", "flightCarries"];
  const emailData = ["Correo"];
  ...
  Object.keys(dataList).forEach((key) => {
    ...
    if (intData.includes(key)) {
      if (!numRegExp.test(dataList[key])) {
        dataCheked.msg += `${key} field only allows Numbers without spaces and must have a minimum of 2 characters</br>`;
        dataCheked.status = false;
      }
    }
    ...
    if (emailData.includes(key)) {
      if (!emailRegex.test(dataList[key])) {
        dataCheked.msg += `${key} field only allows email formats</br>`;
        dataCheked.status = false;
      }
    }
  });
